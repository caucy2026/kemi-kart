...
public:
	...
	//! Returns the element for a specific device (0=Display0, 2=Display2)
	IGUIElement* getHovered(size_t deviceID) const;
...
private:
	...
	void updateHoveredElement(core::position2d<s32> mousePos, size_t deviceID);
	...
	IGUIElement* Hovered[2];
	IGUIElement* HoveredNoSubelement[2];       // subelements replaced by their parent, so you only have 'real' elements here
	IGUIElement* Focus[2];
	core::position2d<s32> LastHoveredMousePos[2];
