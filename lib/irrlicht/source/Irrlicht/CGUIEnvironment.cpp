...
: IGUIElement(EGUIET_ROOT, 0, 0, 0, core::rect<s32>(core::position2d<s32>(0,0), driver ? core::dimension2d<s32>(driver->getScreenSize()) : core::dimension2d<s32>(0,0))),
	Driver(driver), Hovered{0,0}, HoveredNoSubelement{0,0}, Focus{0,0}, LastHoveredMousePos{{0,0},{0,0}}, CurrentSkin(0),
...
{
	if ( HoveredNoSubelement[0] && HoveredNoSubelement[0] != this )
	{
		HoveredNoSubelement[0]->drop();
		HoveredNoSubelement[0] = 0;
	}

	if (Hovered[0] && Hovered[0] != this)
	{
		Hovered[0]->drop();
		Hovered[0] = 0;
	}
...
IGUIElement* CGUIEnvironment::getHovered(size_t deviceID) const
{
	return Hovered[deviceID == 2 ? 1 : 0];
}
...
void CGUIEnvironment::OnPostRender( u32 time )
{
	...
	if ( ToolTip.Element == 0 &&
		HoveredNoSubelement[0] && HoveredNoSubelement[0] != this &&
		...
		HoveredNoSubelement[0]->getToolTipText().size() &&
		getSkin() &&
		...
		)
	{
		...
		pos.UpperLeftCorner = LastHoveredMousePos[0];
		core::dimension2du dim = getSkin()->getFont(EGDF_TOOLTIP)->getDimension(HoveredNoSubelement[0]->getToolTipText().c_str());
		...
		ToolTip.Element = addStaticText(HoveredNoSubelement[0]->getToolTipText(), pos, true, true, this, -1, true);
	...
	if (ToolTip.Element && ToolTip.Element->isVisible() )	// (isVisible() check only because we might use visibility for ToolTip one day)
	{
		...
		if ( !HoveredNoSubelement[0] ||
			!HoveredNoSubelement[0]->isVisible() ||
			!HoveredNoSubelement[0]->getParent()
			)	// got invisible or removed in the meantime?
		{
...
//
void CGUIEnvironment::updateHoveredElement(core::position2d<s32> mousePos, size_t deviceID)
{
	size_t didx = (deviceID == 2) ? 1 : 0;
	
	IGUIElement* lastHovered = Hovered[didx];
	IGUIElement* lastHoveredNoSubelement = HoveredNoSubelement[didx];
	LastHoveredMousePos[didx] = mousePos;

	Hovered[didx] = getElementFromPoint(mousePos);

	if ( ToolTip.Element && Hovered[didx] == ToolTip.Element )
	{
		ToolTip.Element->remove();
		ToolTip.Element->drop();
		ToolTip.Element = 0;
	...
	HoveredNoSubelement[didx] = Hovered[didx];
	while ( HoveredNoSubelement[didx] && HoveredNoSubelement[didx]->isSubElement() )
	{
		HoveredNoSubelement[didx] = HoveredNoSubelement[didx]->getParent();
	}

	if (Hovered[didx] && Hovered[didx] != this)
		Hovered[didx]->grab();
	if ( HoveredNoSubelement[didx] && HoveredNoSubelement[didx] != this)
		HoveredNoSubelement[didx]->grab();

	if (Hovered[didx] != lastHovered)
	{
		SEvent event;
		...
		if ( Hovered[didx] )
		{
			event.GUIEvent.Caller  = Hovered[didx];
			event.GUIEvent.Element = Hovered[didx];
			event.GUIEvent.EventType = EGET_ELEMENT_HOVERED;
			Hovered[didx]->OnEvent(event);
		}
	}

	if ( lastHoveredNoSubelement != HoveredNoSubelement[didx] )
	{
		if (ToolTip.Element)
		{
			ToolTip.Element->remove();
...
bool CGUIEnvironment::postEventFromUser(const SEvent& event)
{
	...
	case EET_MOUSE_INPUT_EVENT:
	{
		...
		size_t devID = (didx == 1) ? 2 : 0;

		updateHoveredElement(core::position2d<s32>(event.MouseInput.X, event.MouseInput.Y), devID);

		if (event.MouseInput.Event == EMIE_LMOUSE_PRESSED_DOWN)
			if ( (Hovered[didx] && Hovered[didx] != Focus[didx]) || !Focus[didx] )
		{
			setFocus(Hovered[didx]);
		}

		if (Focus[didx] && Focus[didx]->OnEvent(event))
			return true;

		if (event.MouseInput.Event == EMIE_MOUSE_WHEEL &&
			Hovered[didx] && Hovered[didx] != Focus[didx])
		{
			if (Hovered[didx]->OnEvent(event))
				return true;
		}

		if (!Focus[didx] && Hovered[didx])
		{
			_IRR_IMPLEMENT_MANAGED_MARSHALLING_BUGFIX;
			return Hovered[didx]->OnEvent(event);
		}
	}
	break;
	...
	case EET_SDL_TEXT_EVENT:
		{
			if (Focus[0] && Focus[0]->OnEvent(event))
				return true;
...
IGUIElement* CGUIEnvironment::getNextElement(bool reverse, bool group)
{
	...
	IGUIElement *startPos = Focus[0] ? Focus[0]->getTabGroup() : 0;
	s32 startOrder = -1;
	...
	if (!group && Focus[0] && !Focus[0]->isTabGroup())
	{
		startOrder = Focus[0]->getTabOrder();
		if (startOrder == -1)
		{
			// this element is not part of the tab cycle,
			// but its parent might be...
			IGUIElement *el = Focus[0];
...
{
	if (Hovered[0] && Hovered[0] == element)
	{
		Hovered[0]->drop();
		Hovered[0] = NULL;
		if (HoveredNoSubelement[0])
