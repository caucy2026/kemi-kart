...
void IrrDriver::update(float dt, bool is_loading)
{
    ...
    else
    {
        ...
        {
            // ═══════════════════════════════════════════════════════════
            //  Each display processes input AFTER syncing its own widgets.
            //  syncDisplayWidgets moves the active widget to (0,0) and
            //  hides the other → position-based hit-testing naturally
            //  isolates touch events per display.
            //  Input is processed inside each pass, NOT globally.
            // ═══════════════════════════════════════════════════════════
            ...
            // ── Pass D2: sync → input → render → swap ──
            if (dualScreenIsReady() && dualScreenMakeCurrent())
            {
                GUIEngine::setCurrentDisplayId(2);
                // 1) Sync: P1 at (0,0) visible, P0 hidden
            ...
            // ── Pass D0: sync → input → render → swap ──
            GUIEngine::setCurrentDisplayId(0);
            // 1) Sync: P0 at (0,0) visible, P1 hidden
            if (GUIEngine::getCurrentScreen())
                GUIEngine::getCurrentScreen()->syncDisplayWidgets(0);
            // 2) Process input — only P0 receives events
            if (input_manager)
                input_manager->update(dt);
