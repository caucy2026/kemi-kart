      ...
      if (m_abort)
      {
        ...
        if (!m_abort)
        {
            ...
            if (!GUIEngine::isNoGraphics())
            {
                ...
                {
                    extern bool g_dual_screen_mode;
                    static int s_loop_log = 0;
                    if (++s_loop_log <= 5)
                        Log::info("main_loop", "Input: g_dual_screen_mode=%d -> %s",
                                  (int)g_dual_screen_mode,
                                  g_dual_screen_mode ? "SKIP" : "RUN");
                    if (!g_dual_screen_mode)
            ...
            {
#ifdef ANDROID
                // Dual-screen: sync widget visibility BEFORE pumping events,
                // so touch events only reach the widget matching this frame's display.
                {
                    extern bool g_dual_screen_mode;
                    if (g_dual_screen_mode && GUIEngine::getCurrentScreen())
                    {
                        static int s_owner_frame = 0;
                        s_owner_frame ^= 1; // alternate 0, 2, 0, 2...
                        int display = s_owner_frame ? 2 : 0;
                        GUIEngine::setCurrentDisplayId(display);
                        GUIEngine::getCurrentScreen()->syncDisplayWidgets(display);
                    }
                }
#endif
