...
// -----------------------------------------------------------------------------
/** Dual-screen: prevent hidden widget's 3D models from being seen by the
 *  visible widget's camera.  Both widgets share the same ISceneManager. */
void ModelViewWidget::setModelVisible(bool visible)
{
    if (m_rtt_main_node != NULL)
        m_rtt_main_node->setVisible(visible);
}   // setModelVisible
