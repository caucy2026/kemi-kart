...
// Dual-screen flag: declared in main.cpp (global scope).
...
{
    Log::verbose("EventHandler", "OnEvent type=%d accept=%d", event.EventType, (int)m_accept_events);
    ...
    */
             ...
             event.EventType != EET_LOG_TEXT_EVENT)
    {
        ...
        if (event.EventType == EET_TOUCH_INPUT_EVENT)
        {
            ...
            static int s_touch_log_count = 0;
            if (++s_touch_log_count <= 50)
                Log::info("EventHandler", "Touch dev=%d X=%d Y=%d curDisp=%d",
                          event.TouchInput.DeviceID,
                          event.TouchInput.X, event.TouchInput.Y,
                          GUIEngine::getCurrentDisplayId());
            ...
            static int s_touch_route_cnt = 0;
            if (++s_touch_route_cnt <= 50)
                Log::info("EventHandler", "postEvent: dev=%d X=%d Y=%d curDisp=%d",
                          mouseEvent.MouseInput.DeviceID,
                          mouseEvent.MouseInput.X, mouseEvent.MouseInput.Y,
                          GUIEngine::getCurrentDisplayId());
            else if (s_touch_route_cnt == 51)
                Log::info("EventHandler", "postEvent: (suppressing further logs)");
    ...
    if (event.EventType == EET_GUI_EVENT)
    {
        ...
        switch (event.GUIEvent.EventType)
        {
            ...
            case EGET_LISTBOX_SELECTED_AGAIN:
            {
                ...
                // Dual-screen: map display to player for independent control.
                {
            ...
            case EGET_ELEMENT_HOVERED:
            {
                ...
                // Debug: log hover events to trace ribbon detection
                static int s_hover_cnt = 0;
                if (++s_hover_cnt <= 30 && w->m_event_handler != NULL)
                    Log::info("EventHandler", "hoverEvent: widget=%s parentType=%d lastTouchDev=%d",
                              w->m_properties[PROP_ID].c_str(),
                              (int)(w->m_event_handler->m_type),
                              m_last_touch_device);
