...
namespace GUIEngine
{
    ...
    int getLargeFontHeight()
   {
    ...
    void update(float dt)
    {
        ...
        static int frame = 0;
        static bool s_logged_accept = false;
        if (frame < 2)
        {
            frame++;
            if (frame == 2)
            {
                ...
                Log::info("GUIEngine", "update: setAcceptEvents(true) after frame=%d", frame);
                s_logged_accept = true;
            }
        }
        if (!s_logged_accept && frame >= 2)
        {
            Log::warn("GUIEngine", "update: frame=%d but setAcceptEvents was never called!", frame);
            s_logged_accept = true;
        }
    }
    // ------------------------------------------------------------------------
