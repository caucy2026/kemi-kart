                           ...
                           const bool pressed, bool focused)
{
    ...
    // Dual-screen: determine which player's selection to show
    int selPlayerID = PLAYER_ID_GAME_MASTER;
#ifdef ANDROID
    {
        if (g_dual_screen_mode) {
            int curDisp = GUIEngine::getCurrentDisplayId();
            selPlayerID = (curDisp == 2) ? 1 : 0;
        }
    }
#endif

    bool mark_selected = widget->isSelected(selPlayerID);

    IGUIElement* focusedElem = NULL;
    if (GUIEngine::getFocusForPlayer(selPlayerID) != NULL)
    {
        focusedElem = GUIEngine::getFocusForPlayer(selPlayerID)
                      ->getIrrlichtElement();
    }
...
#ifdef ANDROID
    // Dual-screen: determine rendering player from current display
    int renderPlayerID = PLAYER_ID_GAME_MASTER;
    if (g_dual_screen_mode) {
        int curDisp = GUIEngine::getCurrentDisplayId();
        renderPlayerID = (curDisp == 2) ? 1 : 0;
    }
    const bool focused =
        GUIEngine::isFocusedForPlayer(widget, renderPlayerID);
#else
    const bool focused =
        GUIEngine::isFocusedForPlayer(widget, PLAYER_ID_GAME_MASTER);
#endif
...
#ifndef SERVER_ONLY
        static int s_rtt_draw_log = 0;
        if (++s_rtt_draw_log <= 20)
            Log::info("Skin", "drawRTTScene: widget=%s curDisp=%d",
